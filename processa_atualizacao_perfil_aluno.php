...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["nome"])) {
    ...
    // Atualize o registro do aluno no banco de dados
    try {
        $sql = "UPDATE Alunos SET nome = :nome WHERE id_aluno = :id_aluno";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':nome', $nome);
        // Faça o bind dos outros parâmetros de atualização aqui
        $stmt->bindParam(':id_aluno', $_SESSION['id_aluno'], PDO::PARAM_INT);
        $stmt->execute();
        
        // Redirecione o usuário de volta para a página do perfil com uma mensagem de sucesso
        header("Location: dash_perfil_aluno.php?success=1");
        exit();
    ...
    header("Location: dash_perfil_aluno.php");
    exit();
}
?>
