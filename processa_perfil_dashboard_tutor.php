...
// Verificar se o tutor está autenticado e a sessão está definida
if (isset($_SESSION['id_tutor']) && !empty($_SESSION['id_tutor'])) {
    // Atribuir o ID do tutor à variável $id_tutor
    $id_tutor = $_SESSION['id_tutor'];

    // Incluir o arquivo de conexão com o banco de dados
    require_once 'conexao.php';

    // Recuperar os dados do tutor do banco de dados
    try {
        $sql = "SELECT nome, email, cidade, estado, data_nascimento, biografia, foto_perfil FROM Tutores WHERE id_tutor = :id_tutor";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':id_tutor', $id_tutor, PDO::PARAM_INT);
        $stmt->execute();
        $dados_tutor = $stmt->fetch(PDO::FETCH_ASSOC);

        // Verificar se os dados do tutor foram encontrados
        if ($dados_tutor) {
            // Atribuir os dados do tutor a variáveis individuais
            $nome = $dados_tutor['nome'];
            $email = $dados_tutor['email'];
            $cidade = $dados_tutor['cidade'];
            $estado = $dados_tutor['estado'];
            $data_nascimento = $dados_tutor['data_nascimento'];
            $biografia = $dados_tutor['biografia'];
            $foto_perfil = $dados_tutor['foto_perfil'];
        } else {
            // Se os dados do tutor não foram encontrados, redirecionar para uma página de erro
            header("Location: pagina_de_erro.php");
            exit();
        }
    } catch (PDOException $e) {
        // Em caso de erro na recuperação dos dados do tutor, exibir uma mensagem de erro
        echo "Erro ao recuperar os dados do tutor: " . $e->getMessage();
        exit();
    }
} else {
    // Se o tutor não estiver autenticado, redirecionar para a página de login
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["editar_perfil"])) {
    ...
    if (isset($_POST['nome']) && isset($_POST['email']) && isset($_POST['cidade']) && isset($_POST['estado']) && isset($_POST['data_nascimento']) && isset($_POST['biografia'])) {
        ...
        // Atualizar os dados do tutor no banco de dados
        try {
            $sql_update = "UPDATE Tutores SET nome = :nome, email = :email, cidade = :cidade, estado = :estado, data_nascimento = :data_nascimento, biografia = :biografia WHERE id_tutor = :id_tutor";
            ...
            $stmt_update->bindParam(':id_tutor', $id_tutor, PDO::PARAM_INT);
            $stmt_update->execute();

            // Redirecionar para o perfil do tutor após a atualização
            header("Location: perfil_tutor.php");
            exit();
        } catch (PDOException $e) {
            // Em caso de erro na atualização dos dados, exibir uma mensagem de erro
            echo "Erro ao atualizar os dados do tutor: " . $e->getMessage();
            exit();
        }
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["excluir_perfil"])) {
    // Excluir o perfil do tutor do banco de dados
    try {
        $sql_delete = "DELETE FROM Tutores WHERE id_tutor = :id_tutor";
        $stmt_delete = $pdo->prepare($sql_delete);
        $stmt_delete->bindParam(':id_tutor', $id_tutor, PDO::PARAM_INT);
        $stmt_delete->execute();
        ...
        echo "Erro ao excluir o perfil do tutor: " . $e-> getMessage();
        exit();
    }
}

// Recuperação dos idiomas associados ao tutor
try {
    ...
    echo "Erro ao recuperar os idiomas do tutor: " . $e->getMessage();
}
?>
