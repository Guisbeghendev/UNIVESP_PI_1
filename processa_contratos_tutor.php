...
session_start();
if (!isset($_SESSION['id_usuario']) || !isset($_SESSION['tipo_usuario']) || $_SESSION['tipo_usuario'] !== 'tutor') {
...
if(isset($_POST['acao_contrato'])) {
    ...
    // Verificar se o contrato pertence ao tutor atual (por segurança)
    $sql_verificar_contrato = "SELECT * FROM Contratos WHERE id_contrato = :id_contrato AND id_tutor = :id_tutor";
    $stmt_verificar_contrato = $conn->prepare($sql_verificar_contrato);
    $stmt_verificar_contrato->bindParam(":id_contrato", $id_contrato);
    $stmt_verificar_contrato->bindParam(":id_tutor", $_SESSION['id_usuario']);
...
// Listar contratos do tutor
$sql_listar_contratos = "SELECT * FROM Contratos WHERE id_tutor = :id_tutor";
$stmt_listar_contratos = $conn->prepare($sql_listar_contratos);
$stmt_listar_contratos->bindParam(":id_tutor", $_SESSION['id_usuario']);
...
header("Location: contratos_tutor.php");
exit();
?>
